...
if(!empty($states)){
    ...
    foreach ($states as $k => $state){
        ...
        switch ($state['key']){
            case "cold":
                $html .= '<div class="state-row '.$class.'"><span class="mark-row__text freeze">'.$state['text'].'</span><svg width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                break;
            case "freeze":
                $html .= '<div class="state-row '.$class.'">';
                $html .= '<span class="mark-row__text cold ">'.$state['text'].'</span>';
...
<div class="tab-contents__card" data-card-id="<?php echo $product['id']; ?>">
    <a href="<?php the_permalink($product['id']);?>" class="card__container-link">
        ...
        <div class="card__marks">
            <div class="marks__mark-row  p-item">
                <?php echo $html;?>
            </div>
        </div>
